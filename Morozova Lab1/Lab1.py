...
class Cat:
    def __init__(self, name: str, height: float, weight: float):
        ...
        :param name: Имя
        :param height: Рост кота
        :param weight: Вес кота
        Примеры:
        >>> pet = Cat('Murzik', 50, 10.5)
        """
        if type(name) != str:
            raise TypeError('Name of cat must be a str')
        self.name = name

        if type(height) not in (int, float):
            raise TypeError('Height of cat must be a number')
        if height <= 0:
            raise ValueError('Height must be positive number')
        self.height = height

        if type(weight) not in (int, float):
            raise TypeError('Weight of cat must be a number')
        if weight <= 0:
    ...
    def eat(self, food: str) -> str:
        ...
        :param food: Пища, которую ест Кот
        :return: Сообщение о приеме пищи
        Примеры:
        >>> pet = Cat('Masyana', 60, 22.1 )
        >>> pet.eat('Fish')
        """
        ...
